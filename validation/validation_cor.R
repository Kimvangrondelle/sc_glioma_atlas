...
pseudobulk <- read.csv("counts_combined_summed.csv", row.names = 1)
...
bulk_tumor <- pseudo_filtered[, "sum_Tumor", drop=FALSE]
bulk_div <- pseudo_filtered[, "sum_Div_tumor", drop=FALSE]
...
sample_tumor <- specificity_sample_filtered[, "GTumor", drop=FALSE]
sample_div <- specificity_sample_filtered[, "GDiv.tumor", drop=FALSE]
...
cor(sample_tumor, bulk_tumor, method = "spearman")
cor(sample_div, bulk_div, method = "spearman")
